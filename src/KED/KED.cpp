...
KED::KED(EMODEL *model, double Kmagnitude_max, double spacing[3], bool is_spacing_auto)
{
...
KED::KED(EMODEL *model, int zone[3], double thickness, double Kmagnitude_max, double spacing[3], bool is_spacing_auto)
{
    printf("[INFO] Starting computation of kinematical electron diffraction...\n");
    radiusK=Kmagnitude_max;
    radiusE=1.0/model->lambda;
...
// void KED::count_diffraction_vectors(EMODEL *model)
// {
//     printf("[INFO] Spacings along three axes in reciprocal space (Angstrom-1): %.8f %.8f %.8f\n", spacingK[0], spacingK[1], spacingK[2]);
...
// void KED::count_diffraction_vectors(EMODEL *model, int zone[3], double thickness)
// {
//     printf("[INFO] Spacings along three axes in reciprocal space (Angstrom-1): %.8f %.8f %.8f\n", spacingK[0], spacingK[1], spacingK[2]);
//     printf("[INFO] Radius of reciprocal sphere (Angstrom-1): %.8f\n", radiusK);
//     int NspacingK[3];
...
void KED::add_k_node(int h, int k, int l, double K[3], double Kmag, double intensity)
{
...
void KED::compute_diffraction_intensity(EMODEL *model)
{
    printf("[INFO] Starting computation of diffraction intensity...\n");
    double K[3]={0.0};
    double intensity=model->get_diffraction_intensity(0.0, K);
    add_k_node(0, 0, 0, K, 0.0, intensity);
...
void KED::compute_diffraction_intensity(EMODEL *model, int zone[3], double thickness)
{
    double n_zone[3]={double(zone[0]), double(zone[1]), double(zone[2])};
    vector_normalize(n_zone, n_zone);
    vector_constant(n_zone, radiusE, n_zone);
...
void KED::vtk(const char *vtk_path, double threshold)
{
...
void KED::ked(const char *ked_path, double threshold)
{
    FILE *fp=nullptr;
    fp=fopen(ked_path,"w");
    int numi=0;
    ...
    ktemp=khead;
    fprintf(fp, "%.8f\t%.8f\t%.8f\t%.8f\t%.8f\n", ktemp->K[0], ktemp->K[1], ktemp->K[2], ktemp->intensity, 100.0);
    ktemp=ktemp->next;
    ...
    fclose(fp);
    printf("[INFO] Information for three-dimensional kinematical electron pattern stored in %s\n", ked_path);
}

void KED::ked(const char *ked_path, char *png_path, int xaxis[3], int yaxis[3], int zaxis[3], double threshold)
{
    double rotation[3][3];
    ...
    fprintf(fp, "# x\ty\tintensity\tintensity_norm (%d points, x-[%.2f %.2f %.2f], y-[%.2f %.2f %.2f], z-[%.2f %.2f %.2f])\n", numi, x[0], x[1], x[2], y[0], y[1], y[2], z[0], z[1], z[2]);
    
    double *kvector_x, *kvector_y, *kintensity;
    callocate(&kvector_x, numi, 0.0); 
    callocate(&kvector_y, numi, 0.0); 
    callocate(&kintensity, numi, 0.0);
    ktemp=khead;
    int counti=0;
    kvector_x[counti]=kvector_y[counti]=0.0; kintensity[counti]=100.0;
    fprintf(fp, "%.8f\t%.8f\t%.8f\t%.8f\n", 0.0, 0.0, ktemp->intensity, 100.0);
    ktemp=ktemp->next;
    counti++;
    double consti=100.0/intensity_max;
    for(int i=1;i<numk&&ktemp!=nullptr;i++){
        if(ktemp->intensity>threshold){
            ...
            fprintf(fp, "%.8f\t%.8f\t%.8f\t%.8f\n", xyz[0], xyz[1], ktemp->intensity, intensity);
            counti++;
            fflush(fp);
        }
        ktemp=ktemp->next;
    ...
    fclose(fp);
    printf("[INFO] Information for kinematical electron pattern stored in %s.\n", ked_path);
    ...
    graph.draw(png_path);
    printf("[INFO] Image for kinematical electron pattern stored in %s\n", png_path);
}
