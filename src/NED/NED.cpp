...
NED::NED(NMODEL *model, double min2Theta, double max2Theta, int lp_type, double spacing[3], bool is_spacing_auto)
{
    printf("[INFO] Starting computation of neutron diffraction...\n");
    if(is_spacing_auto){
    ...
    quick_unique();
    printf("[INFO] Ending computation of neutron diffraction\n");
}
...
void NED::add_k_node(int h, int k, int l, double theta, double intensity, int multiplicity)
{
    if(ktail==nullptr){
        khead=ktail=new NED_KNODE;
        ktail->k[0]=h; ktail->k[1]=k; ktail->k[2]=l;
        ktail->theta=theta; ktail->intensity=intensity;
        ktail->multiplicity=multiplicity;
        numk++;
    }else{
        ktail->next=new NED_KNODE;
        ktail=ktail->next;
        ktail->k[0]=h; ktail->k[1]=k; ktail->k[2]=l;
        ktail->theta=theta; ktail->intensity=intensity;
        ktail->multiplicity=multiplicity;  
...
void NED::copy_knode_data(NED_KNODE *knode1, NED_KNODE *knode2)
{
...
void NED::swap_knode_data(NED_KNODE *knode1, NED_KNODE *knode2)
{
    NED_KNODE *ktemp=new NED_KNODE;
...
void NED::quick_sort(NED_KNODE *kstart, NED_KNODE *kend)
{
    if(kstart==nullptr||kend==nullptr||kstart==kend) return;
    NED_KNODE *knode1=kstart;
    NED_KNODE *knode2=kstart->next;
...
void NED::quick_unique()
{
    quick_sort(khead, ktail);
    int countk=0;
    NED_KNODE *kslow=khead, *kfast=khead;
    while(kfast!=nullptr){
...
void NED::ned(char *ned_path, char *png_path)
{
    ...
    printf("[INFO] Image for neutron pattern stored in %s\n", png_path);
}

void NED::ned(char *ned_path, char *png_path, double peak_parameter, double scherrer_lambda, double scherrer_size, double dt)
{
