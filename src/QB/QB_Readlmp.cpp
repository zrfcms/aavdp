...
{    
    ...
    while(1)
    {
        if(QB_fgets(gc,1024,fi2)==NULL){QB_fclose(fi2);return -1;}
		token=strtok(gc," \t\r\n");
        if(token!=NULL){
    ...
    for(now=0;now<QB->TotalNumber;now++)
    {
		...
		token=strtok(gc," \t\r\n");
        if(token==NULL){SV_wait_end();QB_fclose(fi2);return -2;}
		else QB->atom[now].id=(int)QB_checkdat(token);
		
		token=strtok(NULL," \t\r\n");
        if(token==NULL){SV_wait_end();QB_fclose(fi2);return -2;}
		else QB->atom[now].type=(int)QB_checkdat(token);
		
		token=strtok(NULL," \t\r\n");
        if(token==NULL){SV_wait_end();QB_fclose(fi2);return -2;}
		else QB->atom[now].x=QB_checkdat(token);
		
		token=strtok(NULL," \t\r\n");
        if(token==NULL){SV_wait_end();QB_fclose(fi2);return -2;}
		else QB->atom[now].y=QB_checkdat(token);
		
		token=strtok(NULL," \t\r\n");
        if(token==NULL){SV_wait_end();QB_fclose(fi2);return -2;}
		else QB->atom[now].z=QB_checkdat(token);
    }
