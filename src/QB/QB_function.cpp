...
void QB_combine_system(QB_tools *output,QB_tools *input)
{
	...
	for(i=0;i<input->TotalNumber;i++)
    {
        ...
        output->atom[last_TotalNumber+i].type=output->TypeNumber+input->atom[i].type;
			...
			for(j=0;j<ny;j++)
			   for(k=0;k<nz;k++)
				   for(m=0;m<QB->TotalNumber;m++)      
				   {
						...
						if(QB->atom[m].id<0)QB->atom[n].id=-n-1;
						else QB->atom[n].id=n+1;
						n++;
				   }
	}
	else
...
void QB_rotate(QB_tools *QB,int i,vector center,vector vv, float angle)
{
    angle*=QB_PI/180.0;
    double normalize;
...
void QB_mirror(QB_tools *QB,int i,vector p0, double dis)
{
