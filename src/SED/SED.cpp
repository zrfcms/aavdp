...
void SED::sed(const char *sed_path, int xaxis[3], int zaxis[3], double thickness, double threshold)
{
    double rotation[3][3];
    ...
    double imax=0.0, imin=1.0e8, imt=thickness/2.0;
    for(int i=0;i<numk;i++){
        if(Kintensity[i]>threshold&&fabs(vector_dot(Kvectors[i], z))<imt){
            Kindice[numi]=i;
            numi++;
    ...
    printf("[INFO] Reciprocal space along x-[%.5f %.5f %.5f], y-[%.5f %.5f %.5f], and z-[%.5f %.5f %.5f]\n", x[0], x[1], x[2], y[0], y[1], y[2], z[0], z[1], z[2]);
    printf("[INFO] Number of diffraction intensity: %d\n", numi);
    printf("[INFO] Range of diffraction intensity: %.8f %.8f\n", imin, imax);
    FILE *fp=nullptr;
    fp=fopen(sed_path,"w");
