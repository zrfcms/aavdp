...
{
    nseed=nseed;
    callocate_2d(&default_seeds, nseed, ns, 0);
    int *seeds;
    ...
    for(int i=0;i<nseed;i++){
        for(int j=0;j<ns;j++){
            default_seeds[i][j]=seeds[i*ns+j];
        }
    }
    deallocate(seeds);
}
...
DED_MC::DED_MC(CELL *cell, double omega, double sigma, double Emax, double Emin, double zmax, double zstep, int nume, int nump)
{
    ave_M=cell->ave_M; 
    ave_Z=cell->ave_Z;
...
void DED_MC::compute(int &count_bse, int ne, double dir0[3], double E0, double Ex)
{
...
void DED_MC::update_free_path(double &step, double &alpha, double E, double E0)
{
...
void DED_MC::update_incident_coordinate(double xyz[3], double dir[3], double step)
{
    double sdir[3];
...
void DED_MC::update_incident_energy(double &E, double step)
{
...
void DED_MC::update_incident_direction(double dir[3], double alpha)
{
    double R=rng->random();
...
void DED_MC::img(char *img_path, double dimension, int resolution)
{
    image_array(img_path, accum_E, nump, nump, dimension, dimension, resolution);
    printf("[INFO] Image data for electron distribution stored in %s.\n", img_path);
}